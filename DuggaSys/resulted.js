...
function process() {
	...
	dstr += "<div class='checkbox-dugga'><input name='sortcol' type='radio' class='sortradio' onclick='sorttype(0)' value='0' id='sortcol0_0'><label class='headerlabel' for='sortcol0_0' onclick='myTable.toggleSortStatus("FnameLnameSSN",1)'>Firstname</label></div>";
	...
	if (searchterm != "" && splitSearch != searchterm) {
		...
		for (colname in row) {
			if (colname == "FnameLnameSSN") {
				...
				if (row[colname]["Firstname"] != null) {
					if (row[colname]["Firstname"].toUpperCase().indexOf(searchterm.toUpperCase()) != -1)
						return true;
				}
			}
		}
